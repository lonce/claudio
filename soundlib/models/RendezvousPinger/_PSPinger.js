...
export { CHORD_INTERVALS, CHORD_NAMES, frequenciesForChord, jitterFrequencies };

/**
 * A timing-worklet SoundModel that renders each phase occurrence by asking an
 * ordinary main-thread Ping SoundModel to play. Structural sibling of
 * _PhaseEventPinger.js, built on PlusSimplexPhasor (via
 * plusSimplexPhaseEventProcessor.js) instead of TransitionPhasor -- adds a
 * `weight` parameter (the PS simplex-driven timing-irregularity control,
 * defaulting to 0 so this behaves identically to PhaseEventPinger until
 * dialed up) but is otherwise the same pool/chord/timing-notification
 * machinery, copied verbatim since none of it is phasor-engine-specific.
 *
 ...
 */
export class PSPinger extends BaseSoundWithEvents {
