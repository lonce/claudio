...
- `soundModels/RendezvousPinger/_TransitionPinger.js` is the private child
  implementation. It owns a preallocated Ping pool and translates
...
- `soundModels/RendezvousPinger/RendezvousPinger.js` packages that two-child behavior as a
...
- `soundModels/RendezvousPinger/RendezvousPingerII.js` adds independently stored natural rates,
  two rendezvous phases, and complementary `rendezvous`/`natural` events.
- `rendezvous_ii_test/index.html` is the audible test for the new model.
...
<http://localhost:8080/rendezvous_ii_test/>

`RendezvousPingerII` keeps the same nested lifecycle and analytics, but exposes
two complementary events. `rendezvous` snapshots `rendezvous_freq`,
`rendezvous_phase_1`, `rendezvous_phase_2`, and `transition_dur`. `natural`
snapshots `natural_freq_1`, `natural_freq_2`, and the same duration. The natural
return is a genuinely phase-unconstrained linear rate drift: it preserves the
phase trajectory naturally implied by the changing rate rather than bending
the rate curve toward an arbitrary endpoint phase.

`transition_sharpness` (0-6, default 3) shapes both event trajectories with a
normalized symmetric tanh curve. Zero is exactly linear; increasing values
approach a continuous step centered halfway through the transition. Because
the curve's mean is always one half, sharpness does not alter the nominal total
phase advance, and rendezvous phase targets remain exact.

All five destination parameters are stored controls. Editing one does not
disturb a running trajectory; the relevant event samples it. `play()` always
starts both phasors at phase zero and at their current natural frequencies.
